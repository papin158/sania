...
class Main:
    ...
    def start(self):
        ...
        auto_b_outvac = tk.Button(self.root, text="Верифицировать В (!Без вакансий!)", command=self.functions.auto_b_without_vac, fg="red")
        auto_gms_outvac = tk.Button(self.root, text="Автопредставление GMS", command=self.functions.auto_b_without_vac, fg="red")
        ...
        how_many_vac = tk.Button(self.root, text=how_many_vac_text, command=self.functions.how_many_vac)
        quit = tk.Button(self.root, text="Quit", command=self.root.destroy)
        refresh_pages = tk.Button(self.root, text="Обновить страницы", command=self.functions.refresh_pages_analysing)
        refresh_pages_gms = tk.Button(self.root, text="Обновить GMS", command=lambda: self.functions.refresh_pages_analysing(True))
        re_login = tk.Button(self.root, text="Перезайти на сайт", command=self.functions.re_login, font=('Arial', '10', 'bold'), fg="red")
